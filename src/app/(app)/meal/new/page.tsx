...
function ManualTab({
  ...
  items: NutrientForm[]
  draft: NutrientForm
  onDraftChange: (f: NutrientForm) => void
  onAddItem: () => void
  onRemoveItem: (index: number) => void
}) {
  const set = (k: keyof NutrientForm, v: string) => onDraftChange({ ...draft, [k]: v })
  const canAdd = draft.foodName.trim().length > 0
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {items.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          {items.map((item, i) => (
            <div
              key={i}
              className="flex items-center justify-between bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5"
            >
      ...
      <div className="flex flex-col gap-3 pt-1">
        {items.length > 0 && (
          <p className="text-xs font-medium text-gray-400">次の食材を入力</p>
        )}
        <div>
          <label className="text-xs font-medium text-gray-400 mb-1.5 block">食品名 *</label>
          <input
            type="text" placeholder="例: ハンバーグ" value={draft.foodName}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="number" placeholder="例: 300" step="1" value={draft.calories}
          ...
          <div>
            ...
            <input type="number" placeholder="例: 20" step="0.1" value={draft.protein}
          ...
          <div>
            ...
            <input type="number" placeholder="例: 40" step="0.1" value={draft.carbs}
          ...
          <div>
            ...
            <input type="number" placeholder="例: 10" step="0.1" value={draft.fat}
...
function MealNewInner() {
  ...
  const [items, setItems] = useState<NutrientForm[]>([])   // 確定済み食材リスト
  const [draft, setDraft] = useState<NutrientForm>(EMPTY_FORM) // 入力中の食材
  ...
  // items + 未確定 draft をまとめて保存できるか
  const allToSave = [...items, ...(draft.foodName.trim() ? [draft] : [])]
  const canSave = allToSave.length > 0
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-gray-900 border-b border-gray-800 px-4 pt-12 pb-3 flex items-center gap-3">
        <button type="button" onClick={() => router.back()} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-800">
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto px-4 pb-8 pt-3 bg-gray-900 border-t border-gray-800">
        ...
        <button
          ...
          className="w-full py-4 bg-orange-500 rounded-2xl font-bold text-base shadow-lg shadow-orange-900/40 disabled:opacity-40 active:scale-[0.98] transition-transform flex items-center justify-center gap-2"
        >
          <span className="text-white font-bold text-base">
            {saving
              ? '保存中...'
              : allToSave.length > 1
                ? `記録する（${allToSave.length}品目）`
                : '記録する'
            }
          </span>
